...
#end unused ports
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets example_design/fifo_block_i/axi_10g_ethernet_core_support_layer_i/refclk]

## Timing Constraints
#10Gig is a blackbox create_clock -period 3.103  [get_pins -hier -filter name=~*gthe2_i/RXOUTCLK]
#10Gig is a blackbox create_clock -period 3.103  [get_pins -hier -filter name=~*gthe2_i/TXOUTCLK]
#10Gig is a blackbox create_clock -period 3.103  [get_pins -hier -filter name=~*interface_1*gthe2_i/RXOUTCLK]
#10Gig is a blackbox create_clock -period 3.103  [get_pins -hier -filter name=~*interface_1*gthe2_i/TXOUTCLK]
#unused interfaces
...
###################
## Other constraints
#clk_200 not in use set_false_path -from [get_clocks xphy_refclk_p] -to [get_clocks clk_200]
#clk_200 not in use set_false_path -from [get_clocks clk_200] -to [get_clocks xphy_refclk_p]

#clk250 and 125 not in use set_false_path -from [get_clocks clk_250mhz_mux_x0y1] -to [get_clocks clk_125mhz_x0y1]
#clk250 and 125 not in use set_false_path -from [get_clocks clk_125mhz_x0y1] -to [get_clocks clk_250mhz_mux_x0y1]

#clk_200 not in use set_false_path -from [get_clocks userclk1] -to [get_clocks clk_200]
#clk_200 not in use set_false_path -from [get_clocks clk_200] -to [get_clocks userclk1]

#userclk1 and sysclk not in use set_false_path -from [get_clocks userclk1] -to [get_clocks sys_clk]
#userclk1 and sysclk not in use set_false_path -from [get_clocks sys_clk] -to [get_clocks userclk1]

#axi_clk and sysclk not in use set_false_path -from [get_clocks userclk1] -to [get_clocks axi_clk]
#axi_clk and sysclk not in use set_false_path -from [get_clocks axi_clk] -to [get_clocks userclk1]
